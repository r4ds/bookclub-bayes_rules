...
$X_{1}, X_{2},  ..., X_{p} \rightarrow$ **Predictors**
...
- Structure of the variability: at any value of X, Y will vary **normaly** around $\mu$ with a consistent standard deviation $\sigma$
...
The only hard part was using an equation from chapter 5: 
...
- $\beta_{0} \rightarrow (intercept)$ (Note that this *is* $\beta_{0}$ even though prior is specified for $\beta_{0c}$)
...
[X] Examine trace plot  

[X] Overlay the density plots of each chains
...
# and use add_fitted_draws() from tidybayes
...
- Built a simple Bayesian Normal regression with response and predictor quantitative 
