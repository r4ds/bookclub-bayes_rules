...
- The numerator is generally not an issue since we specify these distributions.

- However, especially with many parameters, it becomes difficult or intractable to compute the denominator f(y) 

$$ f(y) = \int_{\theta_1}\int_{\theta_2} ... \int_{\theta_k}f(\theta)L(\theta | y) d\theta_k ... d\theta_1 d\theta_2 $$

- Solution is to approximate the posterior via simulation.
...
- Method of producing samples:
    ...
    2. Evaluate the numerator at each possible value ($f(\theta)L(\theta | y)$)
...
- Book also gives a second example using Gamma-Poisson 
...
- Uses 4 *chains* and 1000 *samples* of which 1/2 are discarded by default for *burn-in*
...
- Autocorrelation measures the correlation between pairs of Markov chain values that are one *Lag* “steps” apart 
