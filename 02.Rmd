...
```{r, fig.cap="Bayesian knowledge-building diagram for wether or not the article is fake"}
DiagrammeR::grViz("
  digraph {
...
Here $P(B)$ : prior probability of an article to be a fake news
...
$P(A)$ : probability that an article contains an exclamation point in his title

We know that if an article is fake news : 26.67% that the title contains **!** and if it is not fake this is just 2.22%.

$$ P(A|B) = 0.2667 \quad and \quad P(A|B^c = 0.0222)  $$
...
$P(A|B) \quad and \quad P(A|B^c = 0.0222)$ are our likelihood, when we know A (!) we know that getting B (Fake news) is more likely. It is different that our prior probability.

Then we need can calculate the **joint probability** (probability of observing A nd B for example), of each options (just 2 of them): 

$$ P(A \cap B) = P(A|B) P (B) = 0.2667 *0.4 = 0.1067 $$ 

$$ P(A^c \cap B ) = (1 - P(A|B) * P(B)) = (1 - 0.2667) * 0.4 = 0.2993  $$ 

Here $P(B$) is the *marginal probability* of B

```{r}
B <- c(0.1067, 0.2933, 0.4)
Bc <- c(0.0133, 0.5867, 0.88)
Total <- c(0.12, 0.88, 1)
...
$P(A)$ is our normalizing constant.

Ok but we want $P(fake|exlam)$ ie $P(B|A)$
...
Kasparov only won one of six games. This is our data with $L(\pi|y = 1)$. We can calculate it for each $\pi$ value. What is more likely is that $pi$ was 0.2.
...
The normalizing constant, is a **constant** it appears in all posterior calculations. We can works with unnormalizied proabilities or we can divide each unnormalizied probability by the sum of each of them.
