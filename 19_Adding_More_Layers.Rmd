...
prob_draws |> summarize(across(everything(),mean))
```
 
```{r}
to_plot <- prob_draws |> pivot_longer(cols = c('prob_without_tweak', 'prob_with_tweak'), names_to = "tweak", values_to = "prob")

ggplot(data = to_plot) + geom_density(mapping = aes(x=logit(prob), color=tweak))
```
