...
```{r}
prob_draws <- climber_draws_Ama_Dablam |>
  mutate(exp_tweak = rnorm(n(),0, sqrt(sigma_exp))) |>
  mutate(logit = B_0 + 30*age + oxygen_usedTRUE + b_peak,
         prob_without_tweak = invlogit(logit),
         prob_with_tweak = invlogit(logit + exp_tweak))

prob_draws <- prob_draws |> select(prob_without_tweak, prob_with_tweak)
prob_draws_longer <- prob_draws |> pivot_longer(cols = c('prob_without_tweak', 'prob_with_tweak'), names_to = "tweak", values_to = "prob")
prob_draws_longer |> group_by(tweak) |> summarize(mean=mean(prob),median=median(prob),.groups = "drop")
```
 
 The expedition random tweak only effects the mean, the median is invariant for monotonic function like `invlogit`.  Which to report?  Maybe better to report the posterior distribution!
 
```{r}


ggplot(data = prob_draws_longer) + geom_density(mapping = aes(x=logit(prob), color=tweak))
```
