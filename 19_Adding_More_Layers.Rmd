...
```
  level     group         estimate
  <chr>     <chr>            <dbl>
1 AMAD03107 expedition_id -0.00294
2 AMAD03327 expedition_id  3.31  
```
...
```{r}
logit_p = -1.55 +  2.91 - .0475*30 

invlogit(logit_p)
```

48% chance of success. Could also use `posterior_prediction`
...
Less - due to nonlinear link function.
...
climber_draws_Ama_Dablam <- climber_df |>
     select('(Intercept)',age, oxygen_usedTRUE, 'b[(Intercept) peak_name:Ama_Dablam]','Sigma[expedition_id:(Intercept),(Intercept)]')

head(climber_draws_Ama_Dablam)
```

Ok for each of these 20000 draws, we need to compute a random expedition tweak wiht the appropriate sigma, and then compute the probability from the inverse logistic.
