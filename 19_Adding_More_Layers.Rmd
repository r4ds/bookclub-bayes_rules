...
logit_draws_longer <- logit_draws |> pivot_longer(cols = c('without_tweak', 'with_tweak'), names_to = "tweak", values_to = "logit")

ggplot(data = logit_draws_longer) + geom_density(mapping = aes(x=logit, color=tweak))
```

Summary statistics: 
 
```{r}
logit_draws_longer |> group_by(tweak) |> summarize(mean=mean(invlogit(logit)),median=median(invlogit(logit)),.groups = "drop")
```
 
 The expedition random tweak only effects the mean, the median is invariant for monotonic function like `invlogit`.  But the mean is what you want if you want a prediction for probability of success. 
